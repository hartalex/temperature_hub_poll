  ...
  try {
    fetch(config.hubUrl + '/services/list').then(function (response) {
      ...
      if (isArray(services)) {
          ...
          if (element.type !== 'ws') {
            ...
            fetch(element.url).then(function (response) {
              ...
              if (isArray(sensorData)) {
                ...
                logging.log('error', 'Sensor data array not found')
                slackPost.SlackPost('Sensor data array not found').catch(function (error) {
                  logging.log('error', 'error calling slack', error)
                })
              }
            }).catch(function (err) {
        ...
        logging.log('error', 'Services data array not found')
        slackPost.SlackPost('Services data array not found').catch(function (error) {
          logging.log('error', 'error calling slack', error)
        })
      }
    }).catch(function (err) {
